#!/usr/bin/env python3
"""Deterministic metrics used by the experiment 09 analysis pipeline."""
...
def pair_metrics(
    ...
    if not 0.0 < d_collision < d_violation < r_iapf:
        raise ValueError(
            "thresholds must satisfy 0 < d_collision < d_violation < r_iapf")
    ...
    for uav_a, uav_b in combinations(sorted(positions), 2):
        ...
        near_miss = (series >= d_violation) & (series < r_iapf)
        ...
        results.append(PairMetrics(
            ...
            near_miss_duration=integrate_mask(timeline, near_miss),
...
def debug_metrics(
    ...
    for row in rows:
        ...
        position_offset_norms.append(math.sqrt(sum(
            finite_float(row.get(f"position_offset_{axis}", 0.0), "position offset") ** 2
            for axis in "xyz")))
        acceleration_offset_norms.append(math.sqrt(sum(
            finite_float(
                row.get(f"acceleration_offset_{axis}", 0.0),
                "acceleration offset") ** 2
            for axis in "xyz")))
    ...
    active_uav_seconds = 0.0
    for uav_rows in by_uav.values():
        ordered = sorted(uav_rows, key=lambda row: float(row["timestamp"]))
        uav_times = np.asarray(
            [float(row["timestamp"]) for row in ordered], dtype=float)
        uav_active = np.asarray([
            str(row["iapf_active"]).lower() in ("1", "true")
            for row in ordered], dtype=bool)
        active_uav_seconds += integrate_mask(uav_times, uav_active)
        was_active = False
        was_saturated = False
        ...
        for row in ordered:
            ...
            saturated_now = (
                str(row["position_saturated"]).lower() in ("1", "true")
                or str(row["acceleration_saturated"]).lower() in ("1", "true"))
            ...
            if saturated_now and not was_saturated:
                saturation_events += 1
            was_active = active_now
            was_saturated = saturated_now
    mission_time = float(times[-1] - times[0]) if len(times) > 1 else 0.0
    active_ratio = (
        active_uav_seconds / (mission_time * len(by_uav))
        if mission_time > 0.0 and by_uav else 0.0)
    stale = sum(int(float(row.get("stale_neighbor_count", 0))) for row in rows)
    valid = sum(int(float(row.get("valid_neighbor_count", 0))) for row in rows)
    return {
        "iapf_activation_time": active_uav_seconds,
        "iapf_active_duration": active_uav_seconds,
        "iapf_activation_ratio": active_ratio,
        "iapf_active_ratio": active_ratio,
        "mean_repulsion_norm": (
            float(np.mean(raw_norms)) if raw_norms else 0.0),
        "max_repulsion_norm": max(raw_norms, default=0.0),
        "mean_position_offset": float(np.mean(position_offset_norms)),
        "max_position_offset": max(position_offset_norms, default=0.0),
        "mean_acceleration_offset": float(np.mean(acceleration_offset_norms)),
        "max_acceleration_offset": max(acceleration_offset_norms, default=0.0),
        "position_saturation_ratio": position_saturated / len(rows),
        "acceleration_saturation_ratio": acceleration_saturated / len(rows),
        "saturation_count": saturation_events,
        "stale_neighbor_ratio": stale / (stale + valid) if stale + valid else 0.0,
        ...
        "iapf_activation_count": activation_events,
...
def trajectory_metrics(
    ...
    return {
        ...
        "tracking_rmse": (
            float(np.sqrt(np.mean(np.square(deviations))))
            if deviations else math.nan),
...
def outcome_metrics(
    ...
    arrival_time_variance = math.nan
    active_times = [
    ...
    if event_path.is_file():
        ...
        if command_times and set(stable_by_uav) >= set(positions):
            ...
            arrivals = np.asarray([
                stable_by_uav[uav_id] - min(command_times)
                for uav_id in sorted(positions)], dtype=float)
            arrival_time_variance = float(np.var(arrivals))
    return {
...
def analyze_trial(trial_dir: Path) -> tuple[List[PairMetrics], Dict[str, object]]:
    ...
    pairs, distance_series = pair_metrics(
        ...
        float(thresholds["d_violation"]), float(thresholds["r_iapf"]))
    ...
    summary: Dict[str, object] = {
        ...
        "variant": metadata["method"],
        "assignment_mode": metadata["assignment_mode"],
        "avoidance_mode": metadata["avoidance_mode"],
        "trial": metadata["trial"],
        "seed": metadata["seed"],
        "minimum_inter_agent_distance": closest.minimum_distance,
        "actual_min_distance": closest.minimum_distance,
        ...
        "violation_duration": sum(
            pair.risk_exposure_time for pair in pairs),
        "near_miss_duration": sum(
            pair.near_miss_duration for pair in pairs),
        "risk_integral": sum(pair.risk_integral for pair in pairs),
        "nominal_xy_crossings": metadata["assignment_metrics"]["xy_crossings"],
        "nominal_proximity_crossings": metadata[
            "assignment_metrics"]["proximity_crossings"],
        "predicted_min_distance": metadata["assignment_metrics"]["min_distance"],
        "total_path_length": metadata["assignment_metrics"]["distance"],
        "local_swap_iterations": metadata["assignment_metrics"]["iterations"],
