#!/usr/bin/env python3
"""Build and execute the complete paired experiment 09 protocol."""
...
def protocol_arms() -> List[tuple[str, str, str]]:
    scenarios = [
        ("s1", "s1_crossing_4"),
        ("s1", "s1_crossing_8"),
        ("s2", "s2_dense_local_bias"),
        ("s3", "s3_staggered_dynamic_crossing"),
    ]
    variants = ["B0", "P", "E", "Full"]
    arms = [
        (family, scenario, variant)
        for family, scenario in scenarios
        for variant in variants
    ]
    if len(arms) != 16:
        raise AssertionError(f"expected 16 protocol arms, got {len(arms)}")
    return arms
...
def trial_command(
    ...
    representative_seed = int(
        defaults["experiment"]["representative_seed"])
    if trial.phase == "pilot" or trial.seed != representative_seed:
...
def main() -> int:
    ...
    if batch_dir.exists() and not args.resume:
        raise FileExistsError(
            f"refusing to reuse {batch_dir}; pass --resume to continue")
