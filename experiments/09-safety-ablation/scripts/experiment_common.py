#!/usr/bin/env python3
"""Configuration, pairing, and assignment helpers for experiment 09."""
...
RESULTS_ROOT = Path(os.environ.get(
    "EXPERIMENT_09_RESULTS_ROOT",
    REPO_ROOT / "experiments" / "results" / "experiments_09"))
...
def load_configuration(
    ...
    methods = load_yaml(CONFIG_ROOT / "variants.yaml").get("variants", {})
    if method_name not in methods:
        raise ValueError(f"unknown variant: {method_name}")
...
def paired_input_digest(
    scenario: Mapping[str, Any], groups: List[Dict[str, Any]], seed: int
) -> str:
    """Hash only inputs that must be identical across the four variants."""
    import json
...
def lfs_metadata(scenario_name: str) -> Dict[str, str]:
    path = CONFIG_ROOT / "lfs" / f"{scenario_name}.json"
