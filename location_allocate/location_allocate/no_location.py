...
- 串行多任务中，后续任务若使用「全部无人机」「同一批无人机」「它们」或省略 UAV 编号，U 必须继承前一任务的 U；不得重新扩展为 ROS 情报中的全部无人机
- 并行分组任务之后若使用「全部无人机」指代前述各组，U 必须取这些分组 U 的并集；只有独立首任务省略 UAV 编号时，才使用 ROS 情报中的全部可用无人机编号
...
【示例4：串行任务继承同一批无人机】
用户指令：无人机1到8号机首先以[0,13.5,5]为中心组成直线编队；全部无人机稳定后，再以同一中心组成圆形编队
ROS信息：当前可用无人机编号: [1,2,3,4,5,6,7,8]，总数: 8
输出：
{
  "lfs_version": "1.0",
  "tasks": [
    {
      "task_id": 1, "U": [1,2,3,4,5,6,7,8], "F": "Line",
      "c": [0.0,13.5,5.0], "r": 1.5, "T": 3.0,
      "m": "normal", "s": 1.0, "q": "hover-and-wait"
    },
    {
      "task_id": 2, "U": [1,2,3,4,5,6,7,8], "F": "Circle",
      "c": [0.0,13.5,5.0], "r": 1.5, "T": 3.0,
