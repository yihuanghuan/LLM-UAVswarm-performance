#!/usr/bin/env python3
"""Supplementary controller enumeration with explicit parking offsets.

This launches the unchanged production controller executable and frozen policy.
Only experiment infrastructure enumeration and initial global offsets differ.
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    def create_nodes(context):
        ids = [int(value) for value in LaunchConfiguration("uav_ids").perform(context).strip("[]").split(",") if value.strip()]
        layout_path = LaunchConfiguration("layout_json").perform(context)
        with open(layout_path, encoding="utf-8") as stream:
            layout = {int(row["uav_id"]): row for row in json.load(stream)["positions"]}
        if set(layout) != set(ids):
            raise RuntimeError("layout/UAV-ID mismatch")
        ...
        for uid in ids:
            ...
            remappings = [
                (f"/uav{uid}/fmu/out/vehicle_odometry", px4_topic(namespace, "fmu/out/vehicle_odometry")),
                (f"/uav{uid}/fmu/out/vehicle_status", px4_topic(namespace, "fmu/out/vehicle_status")),
                (f"/uav{uid}/fmu/in/offboard_control_mode", px4_topic(namespace, "fmu/in/offboard_control_mode")),
                (f"/uav{uid}/fmu/in/trajectory_setpoint", px4_topic(namespace, "fmu/in/trajectory_setpoint")),
                (f"/uav{uid}/fmu/in/vehicle_command", px4_topic(namespace, "fmu/in/vehicle_command")),
            ]
            for other in ids:
                if other != uid:
                    remappings.append((f"/uav{other}/fmu/out/vehicle_odometry", px4_topic(f"px4_{other}", "fmu/out/vehicle_odometry")))
            position = layout[uid]
            nodes.append(Node(
    ...
    policy_default = os.path.join(get_package_share_directory("lfs_policy"), "config", "lfs_policy.paper_current.yaml")
    return LaunchDescription([
        DeclareLaunchArgument("layout_json"),
        DeclareLaunchArgument("uav_ids"),
        DeclareLaunchArgument("lfs_policy_file", default_value=policy_default),
