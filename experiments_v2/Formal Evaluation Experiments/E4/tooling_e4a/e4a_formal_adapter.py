#!/usr/bin/env python3
"""Fail-closed E4A formal-capable exact-trial adapter."""
from __future__ import annotations
import json,os,subprocess
from pathlib import Path
from e4a_trial_registry import *
BRANCH='formal/E4A-formal-adapter-v1'; CONTRACT_BRANCH='formal/E4A-behavioral-distinguishability-v1'; CONTRACT_COMMIT='7289861283a0a5f3de311acd4ff7a27ee108d8ea'; PREFLIGHT='36dba68c6b16681ec98500b49c5a83095de4b634'; BASELINE_TAG='paper-final-sim-v3'; BASELINE='6cf402debf23851b1eff3edc6f3ab49eae7127c4'; ENTRYPOINT='experiments_v2/Formal Evaluation Experiments/E4/tooling_e4a/e4a_formal_adapter.py'
class FormalAdapterError(E4AError): pass
...
def run_exact_trial(t,c):
 ...
 root.mkdir(parents=True);(root/'raw').mkdir();accepted=mode=='formal';status='success';backend={'backend':'E4A_pinned_adapter_spec_rehearsal_v1','physical_execution_performed':False,'scientific_outcomes':None,'nominal_reference_hash':canonical_sha256(s['nominal_reference'])}
 if mode=='formal':
  try:
 ...
 a={'record_type':'E4A_formal_capable_attempt_v1','dataset_class':'formal_evaluation' if accepted else c['dataset_class'],'accepted_formal_result':accepted,'result_notice':None if accepted else 'NOT_FORMAL_RESULT','trial_id':t,'experiment':'E4A','global_trial_position':pos,'attempt_status':status,'replacement_attempt':False,'execution_mode':mode,'execution_spec':s,'backend_result':backend,'adapter':i,'provenance':{'preflight_commit':PREFLIGHT,'baseline_tag':BASELINE_TAG,'baseline_commit':BASELINE,'policy_sha256':POLICY_SHA256,'protocol_sha256':PROTOCOL_SHA256,'registry_sha256':REGISTRY_SHA256,'global_trial_order_sha256':ORDER_SHA256}}
 p=root/'attempt.json';_write(p,a);return {'trial_id':t,'experiment':'E4A','attempt_status':status,'artifact_path':str(p),'artifact_sha256':sha256_file(p),'dataset_class':a['dataset_class'],'accepted_formal_result':accepted,'runner_commit':i['commit'],'runner_source_sha256':i['source_sha256'],'suite_journal_mutated':False}
