...
from e4b_trial_registry import E4BError,canonical
ZERO='0'*64
...
class Journal:
 ...
 def read(self):
  ...
  for n,p in enumerate(sorted(self.path.glob('*-attempt.json')),1):
   if p.name!=f'{n:06d}-attempt.json': raise E4BError('journal gap')
   r=json.loads(p.read_text()); b=dict(r); got=b.pop('record_hash',None); want=hashlib.sha256(canonical(b)).hexdigest()
   if r.get('sequence')!=n or r.get('previous_record_hash')!=prev or got!=want: raise E4BError('chain mismatch')
