#!/usr/bin/env python3
"""One cold E4B direct-command physical execution; no retry/campaign authority."""
from __future__ import annotations
import argparse,hashlib,json,os,signal,subprocess,sys,time
from pathlib import Path
from types import SimpleNamespace
from e4b_runtime_provenance import collect_runtime_provenance,runtime_provenance_gate
...
def env(seed):
 raw=subprocess.check_output(['bash','-lc',f'source /opt/ros/humble/setup.bash && source {INSTALL} && env -0']);e={a.decode():b.decode() for x in raw.split(b'\0') if b'=' in x for a,b in [x.split(b'=',1)]};e.update({'ROS_DOMAIN_ID':'44','RMW_IMPLEMENTATION':'rmw_fastrtps_cpp','FORMAL_GAZEBO_SEED':str(seed)});e['GAZEBO_PLUGIN_PATH']='/opt/ros/humble/lib:'+e.get('GAZEBO_PLUGIN_PATH','');return e
...
def direct(spec,phase,result):
 ...
 ids=spec['uav_ids'];targets=spec['initial_positions_m'] if phase=='stage' else spec['assigned_targets_m'];duration=6.0 if phase=='stage' else spec['duration_s'];mission=int(hashlib.sha256((spec['trial_id']+phase).encode()).hexdigest()[:8],16) or 1;profiles=[]
 ...
 class D(Node):
  def __init__(self):
   super().__init__('e4b_exact_command_driver');self.status={};self.events={i:[] for i in ids};self.pubs={i:self.create_publisher(UAVExecutionCommand,f'/uav{i}/execution_command',10) for i in ids};self.sub=[]
   for i in ids:
  ...
  def commands(self):
   e=ExecutableLFS(uav_ids=tuple(ids),formation={'type':'E4B_exact_targets'},center=(0.,0.,0.),radius=0.,duration=duration,motion_style=spec['style'],safety_factor=spec['safety_s'],trigger_semantics={'mode':'direct'});r=SimpleNamespace(executable_lfs=e,assigned_targets=tuple(tuple(x) for x in targets),profiles=tuple(profiles));return build_task_command_batch(r,mission,1,1,self.get_clock().now().to_msg())
 rclpy.init();n=D();out={'phase':phase,'success':False,'mission_id':mission,'publish_count':{str(i):0 for i in ids},'authority_snapshot':{'assignment_mode':spec['assignment_mode'],'avoidance_mode':spec['avoidance_mode'],'duration_s':duration,'authority_evidence':spec['authority_evidence']}};stable=None
 try:
  ...
  commands=n.commands();out['command_payload']=[{'uav_id':int(c.uav_id),'mission_id':int(c.mission_id),'target':[float(c.target_pos.x),float(c.target_pos.y),float(c.target_pos.z)],'style':c.profile.style,'duration':float(c.profile.duration),'configuration_id':c.profile.configuration_id,'velocity_limit':float(c.profile.velocity_limit),'acceleration_limit':float(c.profile.acceleration_limit),'jerk_limit':float(c.profile.jerk_limit),'omega_c':[float(x) for x in c.profile.omega_c],'omega_o':[float(x) for x in c.profile.omega_o]} for c in commands]
...
def orchestrate(spec,output,result):
 ...
 try:
  ...
  time.sleep(20);p,f=start(['ros2','launch','ladrc_controller','swarm_launch.py',f"uav_ids:=[{','.join(map(str,ids))}]",'control_mode:=ladrc_acceleration',f"avoidance_mode:={spec['avoidance_mode']}",f'lfs_policy_file:={POLICY}'],output/'controllers.log',WORK,e);procs.append(p);logs.append(f)
  ...
  out['attempt_status']='success';out['raw_evidence']={'rosbag':'rosbag','staging':'stage_result.json','interaction':'interaction_result.json','controllers':'controllers.log','authority_runtime_spec':'runtime_spec.json','runtime_provenance':'runtime_provenance.json'}
