#!/usr/bin/env python3
"""Resume the frozen 40-attempt F0-only Family-C qualification order."""
...
GRID = E3_DIR / "E3_v4_family_C_execution_deviation_grid.yaml"
RUNNER = TOOLING / "e3_v4_family_C_qualification.py"
...
def main() -> int:
    ...
    for index, (candidate, condition, seed) in enumerate(order, start=1):
        trial_id = f"E3V4C-{candidate}__{condition}__S{seed}"
        ...
        append_journal(journal, {
            "schema": "E3_v4_family_C_batch_journal_v1",
            "order_index": index,
    ...
    completed = sum(
        any(value.get("attempt_status") == "success" for value in manifests(
            output, f"E3V4C-{candidate}__{condition}__S{seed}"
