...
from campaign_interventions import tooling_bundle
...
class PinnedAdapterLoader:
 def __init__(self,registry=None,checkout_roots=None,effective_pins=None,execution_tooling=None):
  self.registry=registry or load_runner_registry();self.pins=effective_pins or self.registry['runners'];self.execution_tooling=execution_tooling or {};self.checkout_roots=checkout_roots or {};self.modules={}
 ...
 def _load(self,family):
  ...
  e=self.pins[family];root=self.checkout(family)
  ...
  expected_tooling=self.execution_tooling.get(family)
  if expected_tooling is not None:
   dirty=subprocess.check_output(['git','status','--porcelain','--untracked-files=all'],cwd=root,text=True)
   if dirty:raise CampaignError(f'{family} execution checkout is dirty')
   actual=tooling_bundle({path:sha256_file(root/path) for path in expected_tooling['files']})
   if actual!=expected_tooling:raise CampaignError(f'{family} execution-tooling bundle mismatch')
 ...
 def verify_all_checkouts(self):
  ...
  for family in self.registry['required_families']:
   module=self._load(family);identity=(getattr(module,'identity',None) or getattr(module,'adapter_identity'))();pin=self.pins[family];ok=identity['commit']==pin['adapter_commit'] and identity['branch']==pin['adapter_branch'] and identity['source_sha256']==pin['adapter_source_sha256'];expected_tooling=self.execution_tooling.get(family)
   if expected_tooling is not None:ok=ok and identity.get('execution_tooling_bundle_schema')==expected_tooling['schema'] and identity.get('execution_tooling_file_sha256')==expected_tooling['files'] and identity.get('execution_tooling_bundle_sha256')==expected_tooling['bundle_sha256']
