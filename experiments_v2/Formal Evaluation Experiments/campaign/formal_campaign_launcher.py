...
INTERVENTION_AUTHORIZATIONS_PATH=CAMPAIGN_DIR/'formal_intervention_authorizations_v1.json'
...
class FormalCampaignLauncher:
 def __init__(self,mode='rehearsal',run_id=REHEARSAL_ID,run_root=None,loader=None,gate_path=LAUNCH_GATE_PATH,intervention_authorizations_path=INTERVENTION_AUTHORIZATIONS_PATH,e3_checkout=None):
  ...
  self.gate_path=Path(gate_path);self.intervention_authorizations_path=Path(intervention_authorizations_path);self.e3_checkout=Path(e3_checkout or REPO_ROOT.parent/'e3_adapter_worktree').resolve()
  if mode=='formal':
  ...
  records=self.journal.read() if self.run_dir.exists() else []
  if self.mode=='formal':epoch=resolve_effective_pins(self.run_dir,len(records),self.registry,self.gate_path,self.intervention_authorizations_path,self.e3_checkout)
  else:epoch={'epoch':0,'effective_pins':self.registry['runners'],'intervention':None}
  self.epoch=epoch['epoch'];self.effective_pins=epoch['effective_pins'];self.intervention=epoch['intervention']
  tooling={}
  if self.intervention is not None:tooling['E3']=self.intervention['effective_identity']['e3_execution_tooling']
  self.loader=loader or PinnedAdapterLoader(self.registry,effective_pins=self.effective_pins,execution_tooling=tooling)
  checkouts=self.loader.verify_all_checkouts()
 ...
 def validate_state(self):
  ...
  if self.mode=='formal':
   epoch=resolve_effective_pins(self.run_dir,len(records),self.registry,self.gate_path,self.intervention_authorizations_path,self.e3_checkout)
   if epoch['epoch']!=self.epoch or epoch['effective_pins']!=self.effective_pins:raise CampaignError('formal intervention epoch changed; audited launcher restart required')
  if self.loader.verify_all_checkouts()['status']!='PASS':raise CampaignError('adapter checkout verification failed')
  for pos,r in enumerate(records,1):
   ...
   pin=self.registry['runners'][family] if pos<3 else self.effective_pins[family]
   ...
   if a.get('adapter_pin')!=pin:raise CampaignError(f'artifact adapter pin mismatch at {pos}')
   if r.get('adapter_commit')!=pin['adapter_commit'] or r.get('adapter_source_sha256')!=pin['adapter_source_sha256']:raise CampaignError(f'journal adapter pin mismatch at {pos}')
 ...
 def dispatch_next(self,requested_trial_id=None,failure_injection=None):
  with CampaignExecutionLock(self.lock):
   ...
   accepted=self.mode=='formal';pin=self.effective_pins[family];payload={'record_type':'global_pinned_adapter_attempt_envelope_v1','dataset_class':'formal_evaluation' if accepted else 'synthetic_validation','accepted_formal_result':accepted,'result_notice':None if accepted else NOT_FORMAL_RESULT,'global_position':pos,'trial_id':trial,'experiment':family,'attempt_status':descriptor['attempt_status'],'replacement_attempt':False,'adapter_artifact_path':relative,'adapter_artifact_sha256':descriptor['artifact_sha256'],'adapter_pin':pin,'failure_injection_classification':'synthetic_infrastructure_semantics_only' if failure_injection else None}
   write_json_exclusive(envelope,payload);eh=sha256_file(envelope);record=self.journal.append({'record_type':'global_formal_suite_journal_attempt_v1' if accepted else 'global_pinned_rehearsal_suite_journal_attempt_v1','dataset_class':payload['dataset_class'],'accepted_formal_result':accepted,'result_notice':payload['result_notice'],'global_position':pos,'trial_id':trial,'experiment':family,'attempt_status':descriptor['attempt_status'],'replacement_attempt':False,'artifact_path':envelope.relative_to(self.run_dir).as_posix(),'artifact_sha256':eh,'adapter_commit':pin['adapter_commit'],'adapter_source_sha256':pin['adapter_source_sha256']},prior_records=self._records);self._records=[*self._records,record];return record
def main():
